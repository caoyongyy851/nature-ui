<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="自然种子" align="center" prop="points" />
      <el-table-column label="现金账户" align="center" prop="cashs" />
    ...
    <!-- 加款 -->
    <el-dialog
      :title="dialogCashs.title"
      :visible.sync="dialogCashs.show"
      width="500px"
      append-to-body
    >
      <el-form ref="form" :model="cashsForm" :rules="rules" label-width="80px">
        <el-form-item label="加/减金额" prop="cheats">
          <el-input
            type="number"
            v-model="cashsForm.cashs"
            placeholder="请输入金额"
          />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitCashs">确 定</el-button>
        <el-button @click="dialogCashs.show = false">取 消</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleJia(row) {
      this.dialogCashs.show = true;
      this.dialogCashs.title = "加款";
      this.cashsForm = {
        id: row.id,
        type: 1,
      };
    ...
    submitCashs() {
      console.log(JSON.stringify(this.cashsForm));
      modifyCashs(this.cashsForm).then((response) => {
        this.msgSuccess("操作成功");
        this.dialogCashs.show = false;
        this.getList();
      });
    },
  },
